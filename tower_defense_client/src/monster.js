export class Monster {
  constructor(path, monsterImages, imageIndex, level) {
    // 생성자 안에서 몬스터의 속성을 정의한다고 생각하시면 됩니다!
    if (!path || path.length <= 0) {
      throw new Error('몬스터가 이동할 경로가 필요합니다.');
    }

    this.monsterNumber = imageIndex;//Math.floor(Math.random() * monsterImages.length); // 몬스터 번호 (1 ~ 5. 몬스터를 추가해도 숫자가 자동으로 매겨집니다!)
    ...
    this.speed = imageIndex === 5 ? 4 : 2; // 몬스터의 이동 속도
  ...
  init(level) {
    ...
    this.attackPower = 10; // 몬스터의 공격력 (기지에 가해지는 데미지)
  }
  ...
  move(base) {
    if (this.currentIndex < this.path.length - 1) {
      ...
      this.hp = -1000; // 몬스터는 이제 기지를 공격했으므로 자연스럽게 소멸해야 합니다.
      return isDestroyed;
    }
  }
