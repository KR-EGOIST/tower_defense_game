...
3. **클라이언트가 서버로부터 수신하는 이벤트 종류 정의 및 코드 구현 (WebSocket으로 통신)**
    ...
    - 상태 동기화 이벤트
        - 타워
            - 클라이언트에 생성된 타워의 위치를 저장합니다.
            - 
        - 몬스터
            - 몬스터를 잡으면 서버에서 잡은 몬스터의 레벨과 종류를 저장합니다.
            - 몬스터를 10마리 잡으면 몬스터의 레벨이 오르고 골드를 1000원 획득합니다.
            - 게임이 종료될 시 지금까지 잡은 몬스터의 점수를 계산해 클라이언트에서 받은 점수와 비교합니다.
